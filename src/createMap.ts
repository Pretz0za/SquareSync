...
function calculateTimeDeltas(times: number[]): number[] {
	let ts = 0;
	let output: number[] = [];
	times.forEach((time) => {
		output.push(time - ts);
...
function calculateCollisionPoints(timeDeltas: number[]): Point[] {
...
function calculateMapVertices(collisions: Point[]): Point[] {
	let output: Point[] = [];
