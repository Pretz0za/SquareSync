declare const boundingCircleRadius;
declare const circleRadius;
declare const circle;
declare const lerp;
declare var frozen;

let circleOutput: CircleType[] = [];
...
function makeCircle(pattern: NotePattern): CircleType {
	const FPS = 30;
...
class Circle {
	...
	constructor(circle: CircleType) {
		...
		this.sideLength = Math.sqrt(
			2 * boundingCircleRadius * boundingCircleRadius * (1 - Math.cos(theta)),
		);
		this.lastHit = -1000;
		this.color = 0;
		this.opacity = 0;
	...
	draw() {
		...
		let pos = [
			lerp(
				this.vertices[this.lastVertex][0],
				this.vertices[nextVertex][0],
				this.lerpCoefficient,
			),
			lerp(
				this.vertices[this.lastVertex][1],
				this.vertices[nextVertex][1],
				this.lerpCoefficient,
			),
		];
